...
flowchart TD
    %% Start
    Start([💬 Customer reports<br>an issue]) --> Warranty{🧾 Is device<br>under warranty?}

    %% Warranty check
    Warranty -->|✅ Yes| IssueType{🧩 What type<br>of issue?}
    Warranty -->|❌ No| OutOfWarranty{🛠️ What type<br>of issue?}
...
- [Middleware](/oss/javascript/langchain/middleware) for dynamic agent configuration
- [Tools](/oss/javascript/langchain/tools) for state management
- [Short-term memory](/oss/javascript/langchain/short-term-memory) for state persistence
...
5. **Sequential workflows**: Perfect for collecting information in a specific order (warranty → classification → resolution).
...
Use **[supervisor](/oss/javascript/langchain/supervisor)** when:
